...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Run 5 of the 6 modules in parallel (authorship needs to run after)
    ...
    // Authorship runs separately (uses Groq, needs file list)
    const authorshipResult = await analyzeCodeAuthorship(octokit, owner, repo, groq);

    // Fetch repo metadata
    const { data: repoData } = await octokit.repos.get({ owner, repo });

    // Weighted score — weights sum to 1.00
    const weightedScore =
    ...
    // Invert: 100 = worst (imposter), 0 = best (solid engineer)
    ...
    const { data: report } = await supabase
      ...
      .insert({
        ...
        repo_owner: owner,
        repo_name: repo,
        repo_description: repoData.description ?? null,
        repo_language: repoData.language ?? null,
        repo_stars: repoData.stargazers_count,
        repo_forks: repoData.forks_count,
        is_private: repoData.private,
        final_imposter_score: imposterScore,
        verdict_label: verdictLabel,
        commit_quality_score: commitResult.score,
        authorship_score: authorshipResult.score,
        pr_review_score: prResult.score,
        repo_hygiene_score: hygieneResult.score,
        consistency_score: consistencyResult.score,
        testing_cicd_score: testingResult.score,
        findings_json: {
          commit_quality: commitResult,
          authorship: authorshipResult,
          pr_review: prResult,
...
async function analyzeCommitQuality(
  ...
  // Sample up to 20 commits for file/line stats (API cost)
  ...
  if (vagueRatio > 0.3)
    findings.push(
      `${Math.round(vagueRatio * 100)}% of commits have vague single-word messages (e.g. "fix", "update", "wip")`
    );
  if (largeRatio > 0.3)
    findings.push(
      `${Math.round(largeRatio * 100)}% of sampled commits are unusually large (50+ files or 5000+ lines)`
    );
  ...
  if (conventionalRatio > 0.5)
    findings.push(
      `✓ ${Math.round(conventionalRatio * 100)}% of commits follow conventional commit format`
    );
...
// ─── Module 2: Code Authorship Signals (uses Groq) ───────────────────────────
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a senior software engineering reviewer. Respond ONLY with valid JSON. No markdown, no explanation outside the JSON.',
        },
        {
          role: 'user',
...
Look for:
- Consistency of coding style, naming conventions, and patterns across files
- Signs of AI-generated code: overly uniform structure, excessive basic comments, no personal shortcuts or idioms
- Copy-paste patterns: large near-identical blocks across files
- Evidence of real iteration vs assembled-looking code
...
async function analyzePRHabits(
  ...
  if (prs.length === 0) {
    return {
      score: 60,
      findings: ['No pull requests found — likely a solo project or direct commits only'],
    };
  }
  ...
  for (const pr of prs) {
    ...
    if (pr.merged_at && pr.created_at) {
      ...
      if (diffMs < 5 * 60 * 1000) quickMerge++; // < 5 minutes
    }

    if (pr.user?.login === pr.merged_by?.login) selfMerged++;
  }

  // Check reviews on a sample of up to 30 PRs
  ...
  // Fix: use Math.max to avoid dividing by 0 or getting a ratio > 1
  const noReviewRatio = noReview / Math.max(samplePRs.length, 1);

  if (noDescRatio > 0.5)
    findings.push(`${Math.round(noDescRatio * 100)}% of PRs have no description`);
  if (quickMergeRatio > 0.3)
    findings.push(
      `${Math.round(quickMergeRatio * 100)}% of PRs were merged within 5 minutes of opening`
    );
  if (selfMergedRatio === 1)
    findings.push('100% of PRs are self-merged with no external reviewers');
  else if (selfMergedRatio > 0.8)
    findings.push(`${Math.round(selfMergedRatio * 100)}% of PRs are self-merged`);
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a documentation reviewer. Respond ONLY with valid JSON. No markdown.',
            },
            {
              role: 'user',
              content: `Rate this README for a software project. Return JSON: { "score": <0-100>, "findings": ["..."] }
...
${readmeContent}`,
  ...
  // Penalise if source files are dumped directly in root with no structure
  ...
  if (Array.isArray(rootContents)) {
    ...
    if (sourceInRoot.length > 10) {
      findings.push(
        `${sourceInRoot.length} source files dumped directly in root with no folder structure`
      );
      score -= 15;
    }
  }
...
async function analyzeContributionConsistency(
  ...
  // Entire history spans fewer than 3 days
  if (repoAgeDays < 3 && commits.length > 20) {
    findings.push(
      `${commits.length} commits all made within ${Math.round(repoAgeDays)} days — entire project appears to have been uploaded at once`
    );
    score -= 40;
  }
  ...
  if (maxInOneDay > 50) {
    findings.push(
      `${maxInOneDay} commits in a single day — suggests bulk upload or cramming`
    );
    score -= 25;
  }

  // Check for >30-day gaps followed by a burst of >20 commits
  ...
  // Reward steady, long-running contributions
...
async function analyzeTestingAndCICD(
  ...
  const sourceFiles = allFiles.filter(
    (f) =>
      sourceExts.some((ext) => f.endsWith(ext)) && !testPatterns.some((p) => p.test(f))
  );
  ...
  if (testFiles.length === 0) {
    ...
    findings.push(
      `✓ ${testFiles.length} test files found (${Math.round(testRatio * 100)}% test-to-source ratio)`
    );
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a hiring consultant writing brief, plain-English developer assessments. Be direct and specific. Write 2-3 sentences only. No bullet points.',
        },
        {
          role: 'user',
...
Be direct. Mention the 1-2 most significant signals (positive or negative).`,
        },
      ],
      max_tokens: 200,
    });
...
async function getOctokit(
  ...
  if (installationId) {
    const { createAppAuth } = await import('https://esm.sh/@octokit/auth-app@6');
    const auth = createAppAuth({
      appId: Deno.env.get('GITHUB_APP_ID')!,
      privateKey: Deno.env.get('GITHUB_APP_PRIVATE_KEY')!.replace(/\\n/g, '\n'),
      installationId: Number(installationId),
    });
    const { token } = await auth({ type: 'installation' });
    return new Octokit({ auth: token });
  }
  return new Octokit({ auth: userToken ?? Deno.env.get('GITHUB_PAT') ?? undefined });
}
